...
urlFragment: "active-directory-verifiable-credentials-go"
---
...
| handler\issuance.go | This is the file which implements http handlers which contains the API called from the webpage. It calls the REST API after getting an access token through MSAL. |
...
| handler\verification.go | This is the file which implements http handlers which contains the API called from the webpage. It calls the REST API after getting an access token through MSAL and helps verifying the presented verifiable credential.
...
```Powershell
git clone https://github.com/Azure-Samples/active-directory-verifiable-credentials-go.git
cd active-directory-verifiable-credentials-go\1-go-api-idtokenhint
```
...
If you want to modify the payloads `issuance_request_config.json` and `presentation_request_config.json` files yourself, make sure you comment out the code overwriting the values in the handler/issuance.go and handler/verification.go files. The code overwrites the Authority, Manifest and trustedIssuers values. The callback URI is modified in code to match your hostname. You can optionally add a fourth parameter to the run command, which represents the PEM file password in case you were using certificate authentication.

The file [run.cmd](run.cmd) is a template for setting all environment variables and running your Go application.
...
In order to build & run the sample, you need to have [Go](https://go.dev/dl/) installed locally. 


1. After you have edited the file [config.json](config.json), start the Go app by running this in the command prompt
```Powershell
go install
.\run.cmd
```
...
As a first source of information, the Go sample will trace output into the console window of all HTTP calls it receives. Then a good tip is to use Edge/Chrome/Firefox dev tools functionality found under F12 and watch the Network tab for traffic going from the browser to the Go app.
